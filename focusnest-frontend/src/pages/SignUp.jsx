...
function passwordStrengthInfo(p) {
  if (!p) return null;
  ...
  if (score <= 2) return { label: "Weak",   color: "bg-red-400",    text: "text-red-500",    width: "w-1/4"  };
  if (score <= 3) return { label: "Fair",   color: "bg-amber-400",  text: "text-amber-500",  width: "w-1/2"  };
  if (score <= 4) return { label: "Good",   color: "bg-blue-400",   text: "text-blue-500",   width: "w-3/4"  };
...
export default function SignUp() {
  ...
  const strength = passwordStrengthInfo(form.password);
        ...
        <div className="w-full max-w-md" style={{ animation: "fadeSlideUp 0.5s ease both" }}>
          ...
          <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              {pwError && (
                <p className="mt-1.5 text-xs text-red-500 leading-relaxed">{pwError}</p>
              )}
              {form.password && !passwordValid && !pwError && (
            ...
            <button
              type="submit"
              disabled={authLoading || !passwordValid}
            ...
            <p className="text-center text-xs text-slate-400 leading-relaxed">
              ...
              <Link to="/privacy-policy" className="cursor-pointer text-indigo-600 hover:underline">Privacy Policy</Link>
              {" "}and{" "}
              <Link to="/terms-of-use" className="cursor-pointer text-indigo-600 hover:underline">Terms of Use</Link>
            </p>
          </form>
        </div>
      </div>
